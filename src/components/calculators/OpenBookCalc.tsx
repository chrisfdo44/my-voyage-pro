  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 sm:py-12">
      <div className="flex items-center gap-4 mb-8 sm:mb-12">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white tracking-tighter uppercase">Open Book <span className="text-cyan-glow">Audit</span></h1>
          <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mt-1">Freight Adjustment & Cost Breakdown Module</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="glass-panel rounded-3xl overflow-hidden border border-cyan-glow/20">
            <div className="bg-cyan-glow/10 p-4 sm:p-6 border-b border-cyan-glow/10">
              <h3 className="font-bold text-xs uppercase tracking-widest text-cyan-glow">Freight Summary</h3>
            </div>
            <div className="p-4 sm:p-6 space-y-6">
              <div className="flex justify-between items-end">
                <span className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Additional Freight</span>
                <span className="text-lg sm:text-xl font-mono font-bold text-white">${results?.additionalFreight?.toFixed(2) ?? "0.00"}</span>
              </div>
              <div className="pt-6 border-t border-cyan-glow/10">
                <span className="text-cyan-glow text-[10px] font-bold uppercase tracking-widest block mb-2">Adjusted Rate</span>
                <div className="text-3xl sm:text-4xl font-mono font-bold text-white tracking-tighter">${results?.newFreight?.toFixed(2) ?? "0.00"}</div>
              </div>
            </div>
          </div>

          <div className="glass-panel rounded-3xl p-4 sm:p-6 space-y-4">
            <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Operational Metrics</h4>
            ...
            <div className="pt-4 border-t border-cyan-glow/10">
              <Metric label="Extra VLSFO" value={(results ? (results.vlsfoSea + results.vlsfoPort) : 0).toFixed(2)} unit="MT" />
              <Metric label="Extra LSMGO" value={(results ? (results.lsmgoSea + results.lsmgoPort) : 0).toFixed(2)} unit="MT" />
            </div>
            <div className="pt-4 border-t border-cyan-glow/10">
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Total Extra Cost</span>
                <span className="text-base sm:text-lg font-mono font-bold text-cyan-glow">${results?.extraCost?.toLocaleString(undefined, { maximumFractionDigits: 0 }) ?? "0"}</span>
              </div>
            </div>
          </div>
        </div>
...
function Metric({ label, value, unit = "" }: { label: string, value: any, unit?: string }) {
  return (
    <div className="flex justify-between text-xs">
      ...
      <span className="font-mono font-bold text-white">{value} <span className="text-[10px] text-slate-600">{unit}</span></span>
    </div>
  );
}
